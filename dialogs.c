...
DWORD WINAPI MultiDlSingleDownloadThread(LPVOID lpParam) {
    ...
    // Post progress update
    {
        ...
        if (progData) {
            ...
            PostMessageW(ctx->hDialog, WM_MULTI_DL_PROGRESS, 0, (LPARAM)progData);
        }
    }
    ...
    // Get download path
    {
        ...
        if (itemResult) {
            ...
            if (success) {
                // Try to find the downloaded file and add to cache
                wchar_t* videoId = ExtractVideoIdFromUrl(url);
                if (videoId) {
                    wchar_t videoPattern[MAX_EXTENDED_PATH];
                    WIN32_FIND_DATAW findData;
                    HANDLE hFind;
                    wchar_t bestFile[MAX_EXTENDED_PATH] = {0};
                    ...
                    if (hFind != INVALID_HANDLE_VALUE) {
                        do {
                            if (!(findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)) {
                                wchar_t* ext = PathFindExtensionW(findData.cFileName);
                                if (ext && (_wcsicmp(ext, L".mp4") == 0 || _wcsicmp(ext, L".mkv") == 0 ||
                                           _wcsicmp(ext, L".webm") == 0 || _wcsicmp(ext, L".avi") == 0)) {
                                    swprintf(bestFile, MAX_EXTENDED_PATH, L"%ls\\%ls", downloadPath, findData.cFileName);
                                }
                            }
                        ...
                        if (bestFile[0]) {
                            wcscpy(itemResult->filePath, bestFile);
                            AddCacheEntry(GetCacheManager(), videoId, bestFile, L"", bestFile, NULL, 0);
                            if (ctx->hMainWindow) {
                                PostMessageW(ctx->hMainWindow, WM_CACHE_SIZE_UPDATE, 0, 0);
                            }
                        }
                    }
                    SAFE_FREE(videoId);
...
DWORD WINAPI MultiDlCoordinatorThread(LPVOID lpParam) {
    ...
    activeThreads = (HANDLE*)SAFE_MALLOC(sizeof(HANDLE) * ctx->maxConcurrent);
    activeIndices = (int*)SAFE_MALLOC(sizeof(int) * ctx->maxConcurrent);
    activeCount = 0;

    if (!activeThreads || !activeIndices) {
        SAFE_FREE(activeThreads);
        SAFE_FREE(activeIndices);
        PostMessageW(ctx->hDialog, WM_MULTI_DL_ALL_DONE, 0, 0);
        return 1;
    }

    memset(activeThreads, 0, sizeof(HANDLE) * ctx->maxConcurrent);
    memset(activeIndices, -1, sizeof(int) * ctx->maxConcurrent);
    ...
    while (!InterlockedCompareExchange(&ctx->stopRequested, 0, 0)) {
        ...
        for (t = 0; t < ctx->maxConcurrent; t++) {
            if (activeThreads[t]) {
                DWORD waitResult = WaitForSingleObject(activeThreads[t], 0);
        ...
        // Start new download if we have capacity and work
        if (itemToStart >= 0 && activeCount < ctx->maxConcurrent) {
            MultiDlWorkerContext* workerCtx = (MultiDlWorkerContext*)SAFE_MALLOC(sizeof(MultiDlWorkerContext));
            if (workerCtx) {
                workerCtx->batchCtx = ctx;
                ...
                for (t = 0; t < ctx->maxConcurrent; t++) {
                    if (!activeThreads[t]) {
                        HANDLE hThread = CreateThread(NULL, 0, MultiDlSingleDownloadThread, workerCtx, 0, NULL);
                        if (hThread) {
                            activeThreads[t] = hThread;
                            activeIndices[t] = itemToStart;
                            activeCount++;
                        } else {
                            SAFE_FREE(workerCtx);
                        }
        ...
        if (itemToStart < 0 && activeCount == 0) {
            break;
        }
    ...
    // Wait for all remaining threads on stop
    for (t = 0; t < ctx->maxConcurrent; t++) {
        if (activeThreads[t]) {
            WaitForSingleObject(activeThreads[t], 5000);
            CloseHandle(activeThreads[t]);
        }
    }
