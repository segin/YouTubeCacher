...
int main() {
    ...
    printf("\nStarting tests for ClearThreadSafeSubprocessOutput...\n");

    printf("Test 6: Call with NULL context... ");
    ClearThreadSafeSubprocessOutput(NULL);
    printf("Passed (did not crash).\n");

    printf("Test 7: Call with uninitialized context... ");
    ThreadSafeSubprocessContext uninitializedContext;
    memset(&uninitializedContext, 0, sizeof(uninitializedContext));
    uninitializedContext.initialized = FALSE;
    ClearThreadSafeSubprocessOutput(&uninitializedContext);
    printf("Passed (did not crash).\n");

    printf("\nAll ClearThreadSafeSubprocessOutput tests passed successfully!\n");
