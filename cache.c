...
// DJB2 hash function for wide strings
static DWORD HashVideoId(const wchar_t* videoId) {
    if (!videoId) return 0;
...
void CleanupCacheManager(CacheManager* manager) {
    ...
    memset(manager->hashBuckets, 0, sizeof(manager->hashBuckets));
...
BOOL LoadCacheFromFile(CacheManager* manager) {
    ...
    for (int i = 0; i < totalLines; i++) {
        ...
        DWORD hashIndex = HashVideoId(videoId);
        entry->hashNext = manager->hashBuckets[hashIndex];
        manager->hashBuckets[hashIndex] = entry;
...
BOOL RemoveCacheEntry(CacheManager* manager, const wchar_t* videoId) {
    ...
    while (current) {
        if (current->videoId && wcscmp(current->videoId, videoId) == 0) {
            ...
            DWORD hashIndex = HashVideoId(videoId);
            CacheEntry* hashCurrent = manager->hashBuckets[hashIndex];
            CacheEntry* hashPrevious = NULL;
            while (hashCurrent) {
                if (hashCurrent == current) {
                    if (hashPrevious) {
                        hashPrevious->hashNext = hashCurrent->hashNext;
                    } else {
                        manager->hashBuckets[hashIndex] = hashCurrent->hashNext;
                    }
                    break;
                }
...
CacheEntry* FindCacheEntry(CacheManager* manager, const wchar_t* videoId) {
    ...
    DWORD hashIndex = HashVideoId(videoId);
    CacheEntry* current = manager->hashBuckets[hashIndex];
    while (current) {
        if (current->videoId && wcscmp(current->videoId, videoId) == 0) {
            return current;
        }
