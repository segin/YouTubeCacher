...
void CleanupCacheManager(CacheManager* manager) {
    ...
    ThreadSafeDebugOutput(L"YouTubeCacher: CleanupCacheManager - Saving cache before cleanup");
...
BOOL SaveCacheToFile(CacheManager* manager) {
    ThreadSafeDebugOutput(L"YouTubeCacher: SaveCacheToFile - ENTRY");

    if (!manager) {
        ThreadSafeDebugOutput(L"YouTubeCacher: SaveCacheToFile - ERROR: NULL manager");
        return FALSE;
    }
    ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - Attempting to save to: %ls", manager->cacheFilePath);
    ...
    if (!file) {
        ...
        ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - ERROR: Failed to open file for writing (error %lu): %ls", error, manager->cacheFilePath);
        return FALSE;
    }

    ThreadSafeDebugOutput(L"YouTubeCacher: SaveCacheToFile - File opened for writing");
    ...
    ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - Writing %d entries", manager->totalEntries);
    ...
    while (current) {
        ...
        if (ValidateCacheEntry(current)) {
            ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - Writing entry %d: %ls", entryCount, current->videoId ? current->videoId : L"NULL");
            ...
            ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - Skipping invalid entry %d: %ls", entryCount, current->videoId ? current->videoId : L"NULL");
        }
    ...
    ThreadSafeDebugOutputF(L"YouTubeCacher: SaveCacheToFile - COMPLETE: Wrote %d entries to file", entryCount);
...
DeleteResult* DeleteCacheEntryFilesDetailed(CacheManager* manager, const wchar_t* videoId) {
    ...
    if (entry->mainVideoFile) {
        if (!DeleteFileW(entry->mainVideoFile)) {
            ...
            ThreadSafeDebugOutputF(L"Failed to delete video file: %ls (Error: %lu)", entry->mainVideoFile, error);
        } else {
            result->successfulDeletes++;

            // Log successful deletion
            ThreadSafeDebugOutputF(L"Deleted video file: %ls", entry->mainVideoFile);
        }
    }
    ...
    for (int i = 0; i < entry->subtitleCount; i++) {
        if (entry->subtitleFiles && entry->subtitleFiles[i]) {
            if (!DeleteFileW(entry->subtitleFiles[i])) {
                ...
                // Log deletion failure with enhanced context
                ThreadSafeDebugOutputF(L"Failed to delete subtitle file: %ls (Error: %lu)", entry->subtitleFiles[i], error);
            } else {
                result->successfulDeletes++;

                // Log successful deletion
                ThreadSafeDebugOutputF(L"Deleted subtitle file: %ls", entry->subtitleFiles[i]);
            }
        }
    }
    ...
    if (result->errorCount == 0) {
        ...
        if (entry->title) {
            ThreadSafeDebugOutputF(L"Removed cache entry for video: %ls (ID: %ls)",
                    entry->title, videoId);
        } else {
            ThreadSafeDebugOutputF(L"Removed cache entry for video ID: %ls", videoId);
        }
...
void InitializeCacheListView(HWND hListView) {
    ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - ENTRY");

    if (!hListView) {
        ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - NULL hListView");
        return;
    }

    ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - Setting extended styles");
    ...
    ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - Adding Title column");
    ...
    ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - Adding Duration column");
    ...
    ThreadSafeDebugOutputF(L"YouTubeCacher: InitializeCacheListView - Column results: %d, %d", result1, result2);

    ThreadSafeDebugOutput(L"YouTubeCacher: InitializeCacheListView - COMPLETE");
}
...
wchar_t* ExtractVideoIdFromUrl(const wchar_t* url) {
    ...
    ThreadSafeDebugOutputF(L"YouTubeCacher: ExtractVideoIdFromUrl - Processing URL: %ls", url);
    ...
    if (vParam) {
        ...
        if (count == 11) {
            ...
            if (videoId) {
                ...
                videoId[11] = L'\0';
                ThreadSafeDebugOutputF(L"YouTubeCacher: ExtractVideoIdFromUrl - Found video ID (v= format): %ls", videoId);
                return videoId;
            }
        }
    }
    ...
    if (shortUrl) {
        ...
        if (count == 11) {
            ...
            if (videoId) {
                ...
                videoId[11] = L'\0';
                ThreadSafeDebugOutputF(L"YouTubeCacher: ExtractVideoIdFromUrl - Found video ID (youtu.be format): %ls", videoId);
                return videoId;
            }
        }
    }
    ...
    if (shortsUrl) {
        ...
        if (count == 11) {
            ...
            if (videoId) {
                ...
                videoId[11] = L'\0';
                ThreadSafeDebugOutputF(L"YouTubeCacher: ExtractVideoIdFromUrl - Found video ID (shorts format): %ls", videoId);
                return videoId;
            }
        }
    }

    ThreadSafeDebugOutput(L"YouTubeCacher: ExtractVideoIdFromUrl - No video ID found in URL");
    return NULL;
}
...
BOOL ValidateCacheEntry(const CacheEntry* entry) {
    if (!entry || !entry->videoId || !entry->mainVideoFile) {
        ThreadSafeDebugOutput(L"YouTubeCacher: ValidateCacheEntry - NULL entry or missing required fields");
        return FALSE;
    }
    ...
    if (!fileExists) {
        ...
        ThreadSafeDebugOutputF(L"YouTubeCacher: ValidateCacheEntry - File validation failed: %ls", entry->mainVideoFile);
    }
