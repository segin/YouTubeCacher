...
static BOOL SaveCacheToFileInternal(CacheManager* manager) {
    ...
    wchar_t debugMsg[1024];
    swprintf(debugMsg, 1024, L"YouTubeCacher: SaveCacheToFileInternal - Attempting to save to: %ls", manager->cacheFilePath);
    ThreadSafeDebugOutput(debugMsg);
    ...
    swprintf(debugMsg, 1024, L"YouTubeCacher: SaveCacheToFileInternal - COMPLETE: Wrote %d entries to file", entryCount);
    ThreadSafeDebugOutput(debugMsg);
